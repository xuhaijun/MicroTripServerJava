...
5. ✅ **已补充集成测试**：`ApiContractTest` 15 用例覆盖鉴权/轨迹/识物/登出吊销/封禁/404/405，
   ...
   - **另有 `StatsClientContractTest`（6 用例）——客户端字段契约测试**：`/trajectory/stats` 的
     `CloudStats.fromMap` 是「按字段名取值、缺字段降级为 0」的**宽容解析**（稳定性所需），
     代价是服务端一旦改名或把数值改成字符串，App **不报错、只是静默显示 0**，UI 上极难发现。
     该测试把客户端认识的 **11 个字段名 + JSON 类型 + 空数据的 `NON_NULL` 省略行为**
     钉死在服务端侧：任一侧变更都会让 `mvn test` 变红，无需人工跑模拟器才能察觉。
     契约以**两端各自独立声明的字面量**表述（刻意不共享常量，否则改名时会一起改、测不出问题）；
     服务端多返回的字段走显式白名单 `SERVER_ONLY_ALLOWLIST`，形成一次「有意识的对齐」。
     原手工脚本 `scripts/verify_stats_contract.py` 保留，仅用于对真实实例做端到端复现。
   - 同类固化：`sync_gpsPointKeyMustBePts_wrongKeySilentlyStoresNothing` 钉住「GPS 点键名必须是 `pts`」——
     传成 `points` 时服务端**返回 200 但静默落库 0 点**，客户端必须读回执 `points` 自检。
6. ✅ **安全加固**：
